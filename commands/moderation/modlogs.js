    ...
    async execute(client, message, args, guildCache) {
        try {
            ...
            if (logs.length == 0) return message.channel.send('There are no logs has been saved!');
                ...
                logs.slice(0, 10).map(a => embed.addField(a.name.slice(0, 1).toUpperCase() + a.name.slice(1), `By: **${message.guild.members.cache.get(a.author)?.user?.tag}** To:  **${message.guild.members.cache.get(a.target)?.user?.tag}**\nReason: **${a?.reason}**`))
