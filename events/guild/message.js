                    ...
                    if (!guildCache.leveling.blacklist.channels.includes(message.channel.id)) {
                        ...
                        if (user.exp > user.level * 300) {
                            ...
                            if (guildCache.leveling.levelUp.enable == true) {
                                ...
                                if (channel) {
                                    let text = guild.leveling.levelUp.text.replace('{userMentions}', `<@${message.author.id}>`).replace('{userName}', message.member.displayName).replace('{server}', message.guild.name).replace('{exp}', user.exp).replace('{level}', user.level)
                                    channel.send(text);
                                }
                            }
                        }
